...
// Local fallback product shape (kept minimal)
type LocalRow = {
  id: string;
  name: string;
  category: string;
...
const staticRows: LocalRow[] = [
  {
    id: "1",
    name: "Bonus Pack 78+8 Diamonds",
    category: "Bonus Pack",
    price: "114",
    status: "Success",
...
export default function RecentOrders() {
  const [rows, setRows] = useState<LocalRow[]>(staticRows);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const res = await codeprojektBackend.getTransactions({ page: 1, limit: 10 });
        if (!mounted) return;
        const mapped: LocalRow[] = res.items.map((t: Transaction) => {
          // Map Transaction -> LocalRow
          // safe accessors for metadata
          const md = t.metadata as Record<string, unknown> | undefined;
          const spuDetails = md?.spuDetails as Record<string, unknown> | undefined;
          const spuName = typeof spuDetails?.name === "string" ? (spuDetails.name as string) : undefined;
          const spuCategory = typeof spuDetails?.category === "string" ? (spuDetails.category as string) : undefined;
          const txnId = typeof md?.transactionId === "string" ? (md.transactionId as string) : undefined;

          const name = spuName || txnId || `Txn ${t.id}`;
          const category = spuCategory || "—";
          const price = typeof t.amount === "number" ? String(t.amount) : "0";
          const status =
            t.substatus?.toLowerCase?.() === "success" || t.status?.toLowerCase?.() === "completed"
              ? "Success"
      ...
      <div className="max-w-full overflow-x-auto">
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            {loading && (
              <TableRow>
                <TableCell className="py-6 text-center" colSpan={4}>
            ...
            {!loading && !error && rows.map((product) => (
              <TableRow key={product.id} className="">
                ...
                <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                  <Badge size="sm" color={product.status === "Success" ? "success" : product.status === "Pending" ? "warning" : "error"}>
                    {product.status}
                  </Badge>
                </TableCell>
              </TableRow>
